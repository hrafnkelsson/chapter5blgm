...
mufcn = 0.5 + 0.15*cos(16*sx - 0.3) + 0.15*cos(16*sy - 0.2);
yvec = exprnd(mufcn,N,1);
muvec = zeros(N,1);
s2vec = zeros(N,1);
qvec = zeros(N,1);

% Compute the mean and variance of the Gaussian approximation of
%
% f = @(x,c) x(1).^2+c.*x(2).^2;  % The parameterized function.
%          c = 1.5;                        % The parameter.
%          X = fminsearch(@(x) f(x,c),[0.3;1])

% Next step: use digamma (psi(x)) and trigamma (psi(1,x)) for new mean 
% and new variance.
alpha = 1;
gamma = 0.5;
exth = - psi(alpha + 1) + log(gamma + yvec(1))
muvec(1)
varth = psi(1,alpha + 1)
s2vec(1)

f = @(t,y,mu,sigma) t  + exp(-t)*y + 0.5*(t - mu)^2/sigma^2;  % The parameterized function.
mu = log(0.5);
sigma = log(1) - log(0.25);
h = 10^(-5);
for j = 1:N;
    y = yvec(j); % The parameter.
    tmax = fminsearch(@(x) f(x,y,mu,sigma),mu);
    s2 = 1/(y*exp(-tmax) + sigma^(-2));   
    s2approx = (h^2)/(f(tmax+h,y,mu,sigma) -2*f(tmax,y,mu,sigma) + f(tmax-h,y,mu,sigma));
    qapprox = 1/s2approx;
    muvec(j) = tmax;
    s2vec(j) = s2approx;
    qvec(j) = 1/s2approx;
end
Qdata = sparse(diag(qvec));

% Test fit
for j = 1:100 %N;
t = -3*log(2):0.01:3*log(2);
pi1 = zeros(length(t),1);
pi2 = zeros(length(t),1);
for k = 1:length(t) 
    pi1(k) = f(t(k),yvec(j),mu,sigma);
    pi2(k) = 0.5*(t(k) - muvec(j))^2/s2vec(j) + f(muvec(j),yvec(j),mu,sigma);
end
plot(t,exp(-pi1),'k-')
hold
plot(t,exp(-pi2),'r-')
...
sigma_nu = 0.25;
Qpost = ((1/sigma_nu^2)*Q1 + Z1'*Qdata*Z1);
mupost = Qpost\(Z1'*Qdata*muvec)
...
figure(2)
surf(xcont,ycont,exp(etapostmatrix)')
view([0 90])
colorbar

% The true field
%mufcn2 = 0.5 + 0.15*(x_bsp_temp - 1) + 0.15*(y_bsp_temp - 0.5) + 0.15*(x_bsp_temp - 1).^2 - 0.10*(y_bsp_temp - 0.5).^2;
mufcn2 = 0.5 + 0.15*cos(16*x_bsp_temp - 0.3) + 0.15*cos(16*y_bsp_temp - 0.2);
...
figure(4)
surf(sx_temp,sy_temp,exp(etapostmatrix)'-(mufcn2matrix)')
view([0 90])
colorbar

ratio_median = median(median(exp(etapostmatrix)'./(mufcn2matrix)'))
log_diff_median = median(median((etapostmatrix)'-log(mufcn2matrix)'))
